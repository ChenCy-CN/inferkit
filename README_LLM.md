...
### 1. 推理性能指标增强 (`llm_engine.py`)
- 新增 **TPOT (Time per Output Token)** 统计：平均 TPOT、中位数 TPOT、P95 TPOT（单位毫秒）
- 返回字典中增加了三个字段：`tpot_mean_ms`、`tpot_median_ms`、`tpot_p95_ms`

### 2. 速度基准测试 (`speed_benchmark.py`)
- 自动用短、中、长三种典型 prompt 反复测试推理速度
- 输出汇总报告：平均 TTFT、生成速度 (tokens/s)、总耗时、输出 Token 数

### 3. Token 剪枝器 (`token_pruner.py`)
- 实现三种剪枝策略：`head_tail`（头尾保留）、`similarity`（相似度去重）、`hybrid`（混合）
- 输入长文本时自动压缩 Token 数量，默认保留 85%
- 返回压缩比、原始/剪枝后 Token 数
